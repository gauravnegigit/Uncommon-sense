...
# INDIA_TIMEZONE is re-exported here because other modules already import it
# from api.appointments; the definition itself now lives in core.timeutils.
from core.timeutils import INDIA_TIMEZONE, today_ist, today_ist_str  # noqa: F401
...
async def book_appointment(
    ...
    if payload.requested_date < today_ist():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def cancel_appointment(
    ...
    # This goes through the same state machine as every other queue change,
    # so a patient already in consultation is never silently cancelled.
    queue_id = doc.get("queue_id")
    ...
    if queue_id:
        ...
        if queue_doc and queue_doc["status"] in {
            ...
            try:
                await _transition_queue_status(
                    db,
                    queue_id,
                    QueueStatus.CANCELLED,
                )
            except HTTPException:
                # The queue entry moved on between the read and the write
                # (the patient was called in). The appointment is still
                # cancelled; the queue keeps its own truth.
                pass
