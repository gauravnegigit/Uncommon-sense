...
async def verify_signup(
    ...
    if not query_filter:
        raise HTTPException(
            ...
            detail="A valid email or phone number is required to verify."
        )
    ...
    # A missing code is a bad request, not a crash: compare against "" so an
    # omitted otp field fails validation instead of raising AttributeError.
    if email:
        submitted_email_otp = (payload.email_otp or "").strip()
        expected_email_otp = pending.get("email_otp") or ""

        if not submitted_email_otp or not secrets.compare_digest(
            expected_email_otp, submitted_email_otp
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if phone:
        submitted_phone_otp = (payload.phone_otp or "").strip()
        expected_phone_otp = pending.get("phone_otp") or ""

        if not submitted_phone_otp or not secrets.compare_digest(
            expected_phone_otp, submitted_phone_otp
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def login(
    ...
    role = (payload.role or "PATIENT").upper()
