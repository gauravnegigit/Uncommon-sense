...
# Upper bound on documents scanned when filtering by a derived display status,
# so a large facility cannot turn one request into an unbounded collection read.
MAX_FOLLOWUP_SCAN = 2000
...
async def list_followups(
    ...
    # UPCOMING / DUE / OVERDUE are derived from the due date, so they cannot
    # be expressed as a Mongo filter. When one is requested, paginate AFTER
    # filtering — otherwise a page can come back short (or empty) simply
    # because the first `limit` documents happened to be a different status.
    if display_status:
        stored_status = {
            FollowUpDisplayStatus.COMPLETED: FollowUpStoredStatus.COMPLETED,
            FollowUpDisplayStatus.CANCELLED: FollowUpStoredStatus.CANCELLED,
        }.get(display_status)

        if stored_status:
            query["status"] = stored_status.value
        else:
            # UPCOMING / DUE / OVERDUE are all open follow-ups.
            query["status"] = FollowUpStoredStatus.OPEN.value

        cursor = (
            db.followups
            .find(query)
            .sort("due_date", 1)
            .limit(MAX_FOLLOWUP_SCAN)
        )
        matching = [
            item
            for item in [followup_response(doc) async for doc in cursor]
    ...
    return [followup_response(doc) async for doc in cursor]
...
async def get_facility_dashboard(
    ...
    # Queue dates are written in IST, so the dashboard must read them in IST.
